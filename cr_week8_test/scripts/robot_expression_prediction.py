...
def implement(data):
	...
	if O =='0' and HA == '0' and HE == '0':
		query = robot_bbn.query()
	elif O == '0' and HA == '0':
		query = robot_bbn.query(HE=HE)
	elif O == '0' and HE == '0':
		query = robot_bbn.query(HA=HA)
	elif HA == '0' and HE == '0':
		query = robot_bbn.query(O=O)
	elif HE == '0':
		query = robot_bbn.query(O=O, HA=HA)
	elif HA == '0':
		query = robot_bbn.query(O=O, HE=HE)
	elif O == '0':
		query = robot_bbn.query(HA=HE, HE=HE)

	perc = {n[1]:v for n,v in query.items() if n[0]=='RE'}	
